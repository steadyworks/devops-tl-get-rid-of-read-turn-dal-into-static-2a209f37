...
def generate_crud_schemas(model_cls: type[SQLModel], name: str) -> tuple[str, bool]:
    ...
    lines.append(f"class {name}Read(ReadableModel):")
    if read_fields:
...
if __name__ == "__main__":
    ...
    for model_cls, name in all_cls:
        class_def, model_uses_field = generate_crud_schemas(model_cls, name)
