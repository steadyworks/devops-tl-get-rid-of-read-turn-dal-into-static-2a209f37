...
class JobManager:
    ...
    async def enqueue(
        ...
        job = await self.jobs_dal.create(
            db_session,
            JobsCreate(
                job_type=job_type,
                status="queued",
    ...
    async def dequeue(
        ...
        updated_job = await self.jobs_dal.update_by_id(
            db_session,
            job_id,
            JobsUpdate(
                status="dequeued",
    ...
    async def update_status(
        ...
        await self.jobs_dal.update_by_id(db_session, job_id, update_data)
        await db_session.commit()
    ...
    async def get_status(
        ...
        job = await self.jobs_dal.get_by_id(db_session, job_id)
        if not job:
