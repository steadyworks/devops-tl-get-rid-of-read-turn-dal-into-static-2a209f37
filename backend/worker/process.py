...
class WorkerProcess(mp.Process):
    ...
    async def _handle_task(
        ...
        try:
            payload = none_throws(job.input_payload)
            assets_dal = AssetsDAL()
            asset_uuids = payload.get("asset_uuids", [])
            asset_objs = await assets_dal.get_by_ids(db_session, asset_uuids)
            image_keys = [obj.asset_key_original for obj in asset_objs]
            logging.info(
                f"[{self.name}] Processing job {job.id} with {len(image_keys)} images"
            )
            await job_manager.update_status(db_session, job.id, "processing")
            processor = JobProcessor(job.id, None, image_keys, asset_manager)
            try:
